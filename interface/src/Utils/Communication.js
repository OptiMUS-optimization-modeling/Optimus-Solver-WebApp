const sendPollingRequest = (
  ...
  // Function to poll the server at /pollResult/<request_id>
  const pollServer = async () => {
    try {
      console.log("Polling server for requestId:", requestId);
  ...
  fetch(process.env.REACT_APP_BACKEND_URL + endpoint, {
    ...
    .then((data) => {
      if (data.received) {
        ...
        throw new Error("Initial request was not received by the server");
      }
    })
    .catch((error) => {
      errorCallback(error);
